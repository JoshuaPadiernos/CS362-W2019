...
int main() {
  ...
  // ----------- TEST 1: Checking If Great Hall Card's Effect Works Properly --------------
  printf("TEST 1: Draw +1 Card, +1 Action\n");
  
  i = numHandCards(&G);
  j = G.numActions;
  printf("Player has %d cards in hand\n", i);
  printf("Player has %d actions available\n", j);
  assert(cardEffect(great_hall, choice1, choice2, choice3, &G, 0, NULL) == 0);
  m = numHandCards(&G);
  l = G.numActions;
  assert(m == i);
